...
void AI_APIENTRY OpenGL::aiEdgeFlagv(const GLboolean *flag)
{
    ...
    if (tracker.is_top_level_api_call() )
    {
        ...
        if (APIInterceptor::get_pre_callback_for_function(APIInterceptor::APIFUNCTION_GL_GLEDGEFLAGV,
                                                         &callback_func_ptr,
                                                         &callback_func_arg) )
        {
            const APIInterceptor::APIFunctionArgument args[] =
            {
                APIInterceptor::APIFunctionArgument::create_u8_ptr(flag),
            };

            callback_func_ptr(APIInterceptor::APIFUNCTION_GL_GLEDGEFLAGV,
