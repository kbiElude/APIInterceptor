...
void AI_APIENTRY OpenGL::aiPassthrough(GLfloat token)
{
    ...
    if (tracker.is_top_level_api_call() )
    {
        ...
        if (APIInterceptor::get_pre_callback_for_function(APIInterceptor::APIFUNCTION_GL_GLPASSTHROUGH,
                                                         &callback_func_ptr,
                                                         &callback_func_arg) )
        {
            const APIInterceptor::APIFunctionArgument args[] =
            {
                APIInterceptor::APIFunctionArgument::create_fp32(token),
            };

            callback_func_ptr(APIInterceptor::APIFUNCTION_GL_GLPASSTHROUGH,
