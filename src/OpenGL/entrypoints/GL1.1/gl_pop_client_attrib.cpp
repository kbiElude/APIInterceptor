...
void AI_APIENTRY OpenGL::aiPopClientAttrib()
{
    ...
    if (tracker.is_top_level_api_call() )
    {
        ...
        if (APIInterceptor::get_pre_callback_for_function(APIInterceptor::APIFUNCTION_GL_GLPOPCLIENTATTRIB,
                                                         &callback_func_ptr,
                                                         &callback_func_arg) )
        {
            callback_func_ptr(APIInterceptor::APIFUNCTION_GL_GLPOPCLIENTATTRIB,
                              0,       /* in_n_args   */
                              nullptr, /* in_args_ptr */
                              callback_func_arg,
                             &should_pass_through);
