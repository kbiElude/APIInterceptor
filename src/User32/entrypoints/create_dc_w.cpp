...
HDC WINAPI User32::create_dc_w(LPCWSTR         in_driver_ptr,
                               LPCWSTR         in_device_ptr,
                               LPCWSTR         in_port_ptr,
                               const DEVMODEW* in_devmode_ptr)
{
     ...
     **/
    AI_TRACE("CreateDCW(pwszDriver=[%p], pwszDevice=[%p], pszPort=[%p], pdm=[%p])\n"
             in_driver_ptr,
             in_device_ptr,
             in_port_ptr,
             in_devmode_ptr);

    if (APIInterceptor::get_pre_callback_for_function(APIInterceptor::APIFUNCTION_USER32_CREATEDCW,
                                                     ...
                                                     &pre_callback_func_arg) )
    {
        const auto devmode_data_chunk_id = APIInterceptor::register_data_chunk(in_devmode_ptr,
                                                                               (in_devmode_ptr != nullptr) ? sizeof(DEVMODEW)
                                                                                                           : 0);
        ...
        pre_callback_func_ptr(APIInterceptor::APIFUNCTION_USER32_CREATEDCW,
    ...
    {
        result = reinterpret_cast<User32::PFNCREATEDCWPROC>(User32::g_cached_create_dc_w_func_ptr)(in_driver_ptr,
                                                                                                   in_device_ptr,
                                                                                                   in_port_ptr,
                                                                                                   in_devmode_ptr);
    }

    if (APIInterceptor::get_post_callback_for_function(APIInterceptor::APIFUNCTION_USER32_CREATEDCW,
                                                      ...
                                                      &post_callback_func_arg) )
    {
        ...
        post_callback_func_ptr(APIInterceptor::APIFUNCTION_USER32_CREATEDCW,
                               post_callback_func_arg,
                               0,       /* in_n_args_out   */
